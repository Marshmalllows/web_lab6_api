...
public class ToastController : ControllerBase
{
    ...
    public IActionResult SaveObject([FromBody] Toast? newToast)
    {
        ...
        return Ok(new
            { message = $"Тост від \"{newToast.Sender}\" з повідомленням \"{newToast.Message}\" успішно збережено!" });
    }
    ...
    [HttpDelete("delete")]
    public IActionResult FlushToasts()
    {
        const string filePath = "file.json";
        if (!System.IO.File.Exists(filePath))
        {
            return NotFound(new { message = "Файл не знайдений!" });
        }

        System.IO.File.Delete(filePath);
        return Ok(new { message = "Тости успішно видалено!" });
    }
